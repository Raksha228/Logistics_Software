...
namespace Backend.DataAccess
{
    public class LogisticData : Logistic, ICrudLogistic
    {
        ...
        // Получение всех логистических записей
        public DataTable Select()
        {
            try
            {
                string apiUrl = $"{_apiBaseUrl}/api/Logistics";
        ...
        // Добавление новой логистической записи
        public bool Insert(Logistic logistic)
        {
            try
            {
                string apiUrl = $"{_apiBaseUrl}/api/Logistics";
                var json = JsonSerializer.Serialize(logistic);
        ...
        // Обновление логистической записи
        public bool Update(Logistic logistic)
        {
            try
            {
                string apiUrl = $"{_apiBaseUrl}/api/Logistics/{logistic.Id}";
                var json = JsonSerializer.Serialize(logistic);
                var content = new StringContent(json, Encoding.UTF8, "application/json");
        ...
        // Поиск логистических записей
        public DataTable Search(string keywords)
        {
            try
            {
                string apiUrl = $"{_apiBaseUrl}/api/Logistics/Search?keywords={Uri.EscapeDataString(keywords)}";
                var response = _httpClient.GetAsync(apiUrl).Result;
        ...
        // Получение продукта для логистики
        public Product GetProductsForLogistic(string keyword)
        {
            try
            {
                string apiUrl = $"{_apiBaseUrl}/api/Products/ForLogistic?keyword={Uri.EscapeDataString(keyword)}";
                var response = _httpClient.GetAsync(apiUrl).Result;
                if (response.IsSuccessStatusCode)
                {
                    var json = response.Content.ReadAsStringAsync().Result;
